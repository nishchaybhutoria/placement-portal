...
6. **Hard outcome gate + cascade across the seasons:** being X-placed blocks applying to any X-outcome job in a dedicated cycle anywhere; accepting an X-outcome offer auto-declines your other X-outcome offers and auto-withdraws your in-flight X-outcome applications **across all dedicated cycles**. Open cycles are exempt in both directions — they neither gate a placed student nor lose their applications to someone else's acceptance (ELG-3.6). The old per-cycle cascade toggles are deleted — this is invariant behavior (ELG-3, OFR-3).
...
6. **Outcome gate — scope depends on the outcome:**
   - **Placement — hard and universal across the seasons, exempt in open cycles:** for a job in a **placement- or internship-kind cycle**, the enrollment holds no accepted, non-terminated placement-outcome offer *anywhere* (any cycle's job, or any external offer — attached or not). An accepted PPO blocks every dedicated-cycle placement job everywhere from the moment it is recorded. **Open-cycle placement jobs carry no gate at applying**: an open cycle is a rolling board rather than a season, so a placed student may apply to it and be offered a role, subject only to the job's own eligibility rule. The gate still stands at **acceptance** — a student is placed in exactly one job — so taking up an open-cycle offer while placed is an explicit, audited move of the placement (OFR-6), not a second acceptance.
...
### OFR-6 — Replacing a placement
A student holds **one** placement, and which one it is can change: a placed student takes an open-cycle role (ELG-3.6), a recorded PPO turns out to be the offer they kept, or the office finds the wrong offer was marked accepted. **Replace placement** (admins only) ends the accepted placement offer and takes up another in **one previewed transaction**: the outgoing offer is terminated as an `admin_correction`, the incoming one is accepted, the OFR-3 cascade runs once against the post-termination state, and the applications the original acceptance auto-declined or auto-withdrew are offered back as the same per-application choices OFR-5 gives. Restoration is narrower here than under OFR-5, and deliberately so: the student is placed before and after, so an application the incoming acceptance would cascade straight back out is **refused rather than restored** — restoring it and undoing it in one commit is not a choice anybody made. What survives is what the new placement does not reach: an open-cycle application, or one of the other outcome. To release those too, terminate the placement under OFR-5 instead. Either side may be a portal offer or an external offer, in any combination.

The outgoing and incoming events each name the other, so the pair reads as one decision rather than two unrelated ones months apart, and a single audit row records both with its reason. The **outgoing** side may live in an archived cycle — a placement recorded last season is exactly the one most likely to need correcting — while the **incoming** side may not, because nothing new is written into an archived cycle. The outcome gate is not overridden but simply not asked: exactly one accepted placement offer exists before and after, so DER-1 never sees two. The student is notified once, describing the move.

This is not the renege review: **OFR-5 → Terminate** remains the path when the point is that an offer ended, with its strike and penalty choices.
