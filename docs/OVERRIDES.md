...
| **UI-20 Replace placement** | Open **Staff → Users → the student**, use **Replace placement** in the Placement card: pick the offer that becomes the placement, give a reason, choose which applications the old acceptance cascaded are restored, and confirm the preview. Admins only; either side may be a portal or an external offer, and the outgoing side may sit in an archived cycle. | `replace_placement` |
...
| 3.3 | Accepted company goes bankrupt/withdraws role | ✅ | ✅ | **UI-12 → Terminate**, choose an appropriate termination kind and restoration candidates. Placement is re-derived, so normal gates resume when no accepted placement remains. When a *replacement* offer already exists, **UI-20** does both halves in one audited transaction instead. |
...
| 3.5 | Accepted PPO, then one dream-company attempt | ✅ | ✅ | **In an open cycle no grant is needed** (ELG-3.6): a placed student applies and is offered normally, and **UI-20** moves the placement if they take the role. In a dedicated cycle, record/accept the PPO with **UI-16**, then grant job+enrollment `outcome_gate` using **UI-5** and apply via **UI-6**. If the PPO is attached to the same capped cycle and a second offer is to be accepted, `offer_cap` may also be required; an earlier PPO acceptance no longer auto-withdraws open-cycle applications, but it does auto-withdraw dedicated-cycle ones. |
| 3.6 | Hold two offers briefly while deciding | ⚠️ | ⚠️ | Two unresolved `offered` rows can coexist already; `offer_cap` limits **accepted** offers and is irrelevant until acceptance. If “hold” means two accepted offers, first acceptance auto-declines the other same-outcome offer. Staff must grant both cycle+enrollment `offer_cap` **and** a suitable `outcome_gate` (**UI-5**), re-extend the auto-declined offer (**UI-12**), then the student accepts it (**UI-11**). There is no atomic “hold two accepted offers without cascade” operation — deliberately: **UI-20** moves a placement rather than holding two, which is the case this scenario is usually standing in for. |
...
2. **Multiple accepted offers (3.6, 3.8):** clarify whether the standard acceptance cascade should be suppressible. Today two grants plus re-extension can create the state, but there is no atomic “hold” action. *Partly closed:* `replace_placement` (**UI-20**) now covers the common case — changing which single offer is the placement — so what remains is genuinely holding two at once.
