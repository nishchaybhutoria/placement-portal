...
**offers/** `extend_offers` staff BULK (from in_progress or pending_offer; schedules expiry) · `accept_offer` student (dedicated cycles; expected_status) · `decline_offer` student · `record_open_outcome` staff BULK (open cycles: →offered|accepted|declined|rejected directly; spec JOB-6) · `terminate_offer` staff (choices: restore-list, discipline, notify; spec OFR-5) · `re_extend_offer` staff · `replace_placement` ADMIN, scope=none (ends one accepted placement offer and takes up another in one tx; either side portal or external; outgoing side exempt from the archived-cycle rule, incoming side not; consults `offer_cap` only — the outcome gate is unasked because exactly one accepted placement survives; linked `offer_terminated`/`accepted` payloads; spec OFR-6)
