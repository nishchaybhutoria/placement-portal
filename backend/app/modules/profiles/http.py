"""Parse-only upload endpoint feeding ``bulk_upsert_profiles`` (Behavior PRO-2).

This route writes nothing: it turns a CSV/XLSX upload into the rows the command
consumes, so the admin previews (``dry_run``) and commits the same parsed data.
"""
...
from app.modules.profiles.parsing import UploadParseError, parse_upload
...
def mount_profile_upload_route(
    app: FastAPI,
    *,
    ...
    @app.post("/api/v1/uploads/profile-rows")
    async def parse_profile_upload(
        ...
        if not (authenticated and actor.role == "admin"):
            raise AuthorizationDenied(authenticated=authenticated)
        await limiter.check(actor.principal_id, "upload_profile_rows", UPLOAD_RATE_LIMIT)
        payload = await file.read(MAX_UPLOAD_BYTES + 1)
        if len(payload) > MAX_UPLOAD_BYTES:
            return _problem("The upload exceeds 5 MB")
        try:
            parsed = parse_upload(payload, file.filename or "")
